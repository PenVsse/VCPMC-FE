...
import { Card, Col, Row, Typography, Space, Checkbox } from "antd";
...
export const MyCard: React.FC<IMyCardProps> = ({ video, isAllChecked }) => {
  return (
    <Col span={6} style={{ position: "relative" }}>
      ...
      >
        ...
        {isAllChecked ? (
          ...
          <Checkbox style={{ position: "absolute", bottom: 12, right: 12 }} />
        )}
      </StyledCard>
    </Col>
  );
...
const UpdateGridDisplay: React.FC<IGridDisplayProps> = ({ data, isAllChecked }) => {
  ...
  return (
    <Row style={{ width: "100%" }}>
      {data.map((video) => (
        <MyCard video={video} key={video.id} isAllChecked={isAllChecked} />
      ))}
    </Row>
  );
};

export default UpdateGridDisplay;
